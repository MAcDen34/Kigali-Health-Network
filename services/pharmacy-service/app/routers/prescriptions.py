...
def list_prescriptions(db: Session = Depends(get_db), actor: ActorContext = Depends(get_current_actor)):
...
def create_prescription(payload: schemas.PrescriptionCreate, db: Session = Depends(get_db), actor: ActorContext = Depends(get_current_actor)):
...
def get_prescription(prescription_id: UUID, db: Session = Depends(get_db), actor: ActorContext = Depends(get_current_actor)):
...
def list_patient_prescriptions(patient_id: UUID, db: Session = Depends(get_db), actor: ActorContext = Depends(get_current_actor)):
