...
export async function listAllPrescriptions() {
  const response = await fetch(`${PHARMACY_API_BASE}/prescriptions`, { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`Failed to fetch prescriptions: ${response.status}`);
  }
  return response.json();
...
export async function listAllInteractionFlags() {
  const response = await fetch(`${PHARMACY_API_BASE}/interaction_flags`, { cache: "no-store" });
  if (!response.ok) {
...
export async function dispensePrescription(prescriptionId, pharmacyId) {
